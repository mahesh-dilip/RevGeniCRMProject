...
/**
 * GET /api/websets/companies/[id]/status
 * Check the status of a company webset
 *
 * Security:
...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      logError('Error checking Exa webset status', exaError, {
        websetId: webset.id,
        exaId: webset.exaId,
      });
    ...
    logError('Error fetching webset status', error);
    return NextResponse.json(
      {
        error: 'Failed to fetch webset status',
