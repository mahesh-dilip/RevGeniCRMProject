...
function AIPeopleFinderContent() {
  ...
  useEffect(() => {
    if (statusData?.status === 'failed') {
      ...
      setStep('review');
      // Auto-select all people as they come in
      ...
      if (searchesComplete && hasNoResults) {
        toast.error('No people found matching your criteria. Try adjusting your search parameters or broadening the location.');
        setStep('search');
      }
    }
  ...
  useEffect(() => {
    if (step === 'review' && previewData?.people) {
      const currentIds = previewData.people.map((p: any) => p.exaId);
      ...
      <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-medium text-blue-900">
                  🔍 Still searching for more contacts...
                </p>
                <p className="text-sm text-blue-700">
                  Results are being added in real-time. Enrichments (email, phone, etc.) are being processed in the background.
                </p>
              </div>
            </div>
          </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-green-700">
                  All contacts have been discovered. Enrichments may still be processing in the background.
                </p>
              </div>
            </div>
          </Card>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              {isStillSearching ? `Found ${people.length} contacts so far...` : `Found ${people.length} contacts.`} Select which ones to import to your CRM.
            </p>
          </div>
          <Button variant="outline" onClick={handleBackToSearch}>
            ← New Search
